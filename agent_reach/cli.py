...
def _cmd_install(args):
    ...
    if args.proxy:
        if dry_run or safe_mode:
            ...
            print("✅ Proxy saved (used when the Agent accesses restricted networks)")
    ...
    if server_skipped_desktop_channels:
        print()
        print("  -- These channels need a desktop environment + Chrome, skipping on a server environment: "
    ...
    if env == "server":
        ...
        print("   Reddit requires a logged-in session (rdt-cli + cookies; see the doctor hints), and mainland China networks also need a proxy.")
        print("   Save a proxy for the Agent: agent-reach configure proxy (hidden input)")
    ...
    if not dry_run:
        ...
        if safe_mode:
            ...
            # Star reminder
            print()
            print("If Agent Reach helped you, leave a Star so more people can find it:")
            print("   https://github.com/Panniantong/Agent-Reach")
            print("   It takes one second and means a lot to an independent developer. Thank you!")
            if not install_ok:
                raise SystemExit(1)
    else:
        print()
...
def _install_system_deps():
    ...
    if shutil.which("deno"):
        ...
        if (
            ...
            print(
                "  -- Did not write the yt-dlp JS runtime config: yt-dlp is missing, too old, or "
                "the version could not confirm. Please upgrade first: python -m pip install -U "
                '"yt-dlp[default]"'
            )
        else:
...
def _install_boss_deps():
    ...
    print("Setting up Boss Zhipin (boss-agent-cli upstream pinned commit)...")
    for tool, args in [
        ...
        try:
            ...
            if result.returncode == 0 and shutil.which("boss"):
                ...
                print("  Next: start the dedicated Chrome, have the user log in to zhipin.com manually, then run agent-reach doctor")
                return True
        except (OSError, subprocess.TimeoutExpired):
            pass
...
def _install_xhs_deps():
    ...
    if _detect_environment() == "server":
        print("  On a server, xiaohongshu-mcp is recommended:")
        print("    1. Download the binary: https://github.com/xpzouying/xiaohongshu-mcp/releases")
        print("       (suggested location: ~/.agent-reach/tools/)")
        print("    2. Start the service (the first run downloads about 150MB of browser data; wait for it to finish)")
        print("    3. Explicitly export cookies from xiaohongshu.com with Cookie-Editor")
        print("       agent-reach configure xhs-cookies (paste at the hidden input prompt)")
        print("    4. Connect: mcporter config add xiaohongshu http://localhost:18060/mcp --scope home")
    ...
    if xhs_ok:
        print("  ✅ Found an existing xhs-cli; it will stay available as a fallback backend")
...
def _install_opencli_deps():
    ...
    if (
        ...
        print("  Last step (manual; Chrome security restriction): install the browser extension")
...
def _install_reddit_deps():
    ...
    if _detect_environment() != "server":
        ...
        print("  Reddit uses OpenCLI (available if reddit.com is already logged in in the browser)")
        import shutil
        if shutil.which("rdt"):
            print("  ✅ Found an existing rdt-cli; it will stay available as a fallback backend")
        return installed
...
def _cmd_configure(args):
    ...
    if args.key == "proxy":
        ...
        print("  Note: Bilibili uses bili-cli and does not need a proxy on mainland China networks.")
        ...
        if auth_token and ct0:
            ...
            print(
                "  Credentials not verified live: will not run `twitter status`, because upstream "
            ...
            if not shutil.which("twitter"):
                ...
                print(
                    "  Note: a standalone `twitter` command does not read Agent Reach config; "
                    "when using it directly, set TWITTER_AUTH_TOKEN/TWITTER_CT0 explicitly."
                )
        else:
...
def _configure_xhs_cookies(value) -> bool:
    ...
    if value.startswith("["):
        try:
            ...
            if isinstance(parsed, list) and parsed:
                ...
                if ignored_invalid:
                    print(
                        f"  [!] ignored {ignored_invalid} cookies with an invalid format"
                    )
                if not valid_cookies:
                    print(
                        "[X] Cookie-Editor JSON has no valid "
    ...
    if mcporter:
        ...
        try:
            ...
            # "已登录" is the success text printed by xiaohongshu-mcp, not our own copy.
...
def _cmd_uninstall(args):
    ...
    if present_legacy_paths:
        print("  [!] Found optional Twitter legacy credential copies; will not delete automatically:")
        for path in present_legacy_paths:
            print(f"      {path}")
        print("      If you confirm xfetch/bird no longer use them, delete them manually.")
    ...
    if shutil.which("mcporter"):
        ...
        try:
            ...
            print(
                "  [!] Could not safely verify the mcporter config source; "
                "will not delete exa/xiaohongshu entries automatically."
            )
        else:
            for mcp_name in ("exa", "xiaohongshu"):
                ...
                print(
                    ...
                    "Agent Reach; kept. If you confirm it is no longer needed, remove it manually."
                )
...
def _cmd_setup():
            ...
            if "exa" in configured_server_names(r.stdout):
                ...
                if setup_now in ("", "y", "yes"):
                    ...
                    if add_r.returncode == 0:
                        ...
                        print("  [!] Automatic setup failed. Run this manually:")
                        print("     mcporter config add exa https://mcp.exa.ai/mcp --scope home")
        except Exception:
            print("  [!] Could not check the Exa config. Run this manually:")
            print("     mcporter config add exa https://mcp.exa.ai/mcp --scope home")
        print()

    # Step 2: GitHub token
    print("[Optional] GitHub token — raise the API limit")
    print("  Without a token: 60 requests/hour | With a token: 5000 requests/hour")
    print("  Get one: https://github.com/settings/tokens (no scopes required)")
    ...
    print("[Info] Reddit — a logged-in session is required (no zero-config path). Desktop: OpenCLI is recommended; or rdt-cli:")
    print(f"  Install: pipx install '{_RDT_GIT_SOURCE}'")
    print("  Then run: rdt login (log in to reddit.com in the browser first)")
    print()
...
_UPDATE_INSTRUCTIONS = (
    "How to update (recommended: copy this sentence to your AI Agent; it fully updates the package, upstream tools, and skill):\n"
...
def _cmd_watch():
    ...
    if not issues and not update_available:
        print(f"Agent Reach: all healthy ({ok}/{total} channels available, v{__version__} is already the latest)")
        return
    ...
    if update_available:
        ...
        if release_body:
            for line in release_body.strip().split("\n")[:10]:
                print(f"    {line}")
        print("  Update (send one sentence to your Agent for a full update):")
