...
class XiaoHongShuChannel(Channel):
    ...
    def check(self, config=None):
        ...
        return "off", (
            ...
            "       (reuses the Chrome login state; zero-config if you have browsed Xiaohongshu)\n"
            f"  Server: xiaohongshu-mcp: {_MCP_INSTALL_URL}\n"
            "       Login uses only an explicit Cookie-Editor export:\n"
    ...
    def _check_opencli(self):
        ...
        if st.ready:
            return "warn", (
                "OpenCLI bridge connected, but Xiaohongshu login state and the actual command were not verified live; "
    ...
    def _check_mcp(self):
        ...
        if not shutil.which("mcporter"):
            return "warn", (
                "xiaohongshu-mcp is reachable, but mcporter is not installed, so Doctor is not connected "
                "to that service. Install it first: npm install -g mcporter"
            )
        try:
        ...
        if "xiaohongshu" in inspection.server_names:
            return "warn", (
                "xiaohongshu-mcp is reachable and connected in mcporter, but login state not verified, "
                "so note access cannot be claimed from this. If you are not logged in, "
                "export cookies with Cookie-Editor and run agent-reach configure xhs-cookies"
            )
        if inspection.imports_unchecked:
            return "warn", (
                "xiaohongshu-mcp is reachable; xiaohongshu was not found in the local mcporter "
                "config, and editor imports were not expanded, so Doctor has not verified the connection."
            )
        return "warn", (
            "xiaohongshu-mcp is running but mcporter is not connected. Run:\n"
    ...
    def _check_xhs_cli(self):
        ...
        try:
            ...
            return "warn", (
                ...
                "Doctor did not run `xhs status`, which would extract browser cookies automatically."
            )
        if payload is None:
        ...
        try:
            ...
            return "warn", (
                "xhs-cli is installed, but the saved cookies.json could not parse safely; "
                "Doctor did not run `xhs status`, which would extract browser cookies automatically."
            )
        ...
        if isinstance(saved_at, (int, float)) and (
            ...
            return "warn", (
                "xhs-cli is installed, and the saved cookie is older than 7 days; Doctor will not let "
                "upstream read the browser or refresh the file automatically. Update it explicitly with Cookie-Editor."
            )
        return "warn", (
            "xhs-cli is installed and an explicitly saved cookie was detected; to avoid upstream "
            "automatically reading the browser or rewriting the cookie, Doctor does not run `xhs status`, not verified live."
        )

    @staticmethod
    def _xhs_cookie_hint():
        return "warn", (
            "xhs-cli is installed but no usable explicit cookie is available. Do not run "
            "`xhs login/status`, which reads the browser automatically; migrate to xiaohongshu-mcp, "
