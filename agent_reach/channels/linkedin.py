    ...
    def check(self, config=None):
        ...
        if inspection.server_names & _LINKEDIN_SERVER_NAMES:
            if not shutil.which("uvx"):
                return "warn", (
                    "LinkedIn MCP is in the mcporter config, but uvx is not installed, "
                    "so the service cannot be started. Install:\n"
                    f"  {_UV_INSTALL_URL}"
                )
            return "warn", (
                "LinkedIn MCP is in the mcporter config, but Doctor has not started the local "
                "service to verify connectivity, so full availability cannot be claimed from config alone."
            )
        if inspection.imports_unchecked:
            return "warn", (
                "LinkedIn MCP was not found in the local mcporter config; editor "
                "imports are also enabled, and Doctor did not expand them to avoid widening credential reads, so this is not verified."
            )
        return "off", (
