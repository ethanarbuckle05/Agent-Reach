...
def opencli_status(timeout: int = 10) -> OpenCLIStatus:
    ...
    if not version_probe.ok:
        return OpenCLIStatus(
            ...
            broken=True,
            hint=(
                "The opencli command exists but cannot execute (node environment is broken). Reinstall:\n"
    ...
    if not st.extension_connected:
        ...
        if st.extension_installed:
            st.hint = (
                "Found OpenCLI extension files for Chrome/Edge, but the extension is not currently connected; "
                "disk files alone could not confirm that it is loaded or enabled.\n"
            ...
            st.hint = (
                "Found ~/.opencli/extension/ source files, but the files being present does not mean they have already been "
                "loaded as an unpacked extension in Chrome/Edge.\n"
                "  Load and enable that directory on the browser extensions page, then run an opencli command to verify"
            )
        else:
            st.hint = (
...
def opencli_summary(st: OpenCLIStatus) -> str:
    ...
    if st.broken:
        return "OpenCLI cannot execute (node environment is broken)"
    if st.extension_connected:
        return f"OpenCLI available (browser login session, v{st.version})"
    if st.extension_installed:
        return "OpenCLI is installed; extension files detected but not currently connected (could not confirm it is loaded)"
    if st.unpacked_extension_files:
        return "OpenCLI is installed; unpacked source files detected but the browser has not been confirmed to have loaded them"
    if st.daemon_running:
        return "OpenCLI is installed, waiting for the Chrome extension to be installed"
    return "OpenCLI is installed (daemon is not running; auto-start on use; Chrome extension required)"
