...
<p align="center">
  <a href="#quick-start">Quick Start</a> · <a href="#quick-start">English</a> · <a href="docs/README_ja.md">日本語</a> · <a href="docs/README_ko.md">한국어</a> · <a href="#supported-platforms">Platforms</a> · <a href="#design-philosophy">Philosophy</a>
</p>
...
| 🎯 **Boss Zhipin** | CDP health check | Job search + full JD text (dedicated Chrome) | Tell your Agent "Set up Boss Zhipin for me". The Agent opens a dedicated Chrome window; you log in yourself |
...
├── boss.py         → boss-agent-cli (dedicated Chrome over CDP)
├── rss.py          → feedparser
...
| Boss Zhipin | [boss-agent-cli](https://github.com/can4hou6joeng4/boss-agent-cli) | — | Dedicated Chrome over CDP. Tell your Agent "Set up Boss Zhipin for me"; you log in yourself |
...
Agent Reach is designed with safety in mind:

| Measure | What it means |
|------|------|
| 🔒 **Credentials stay local** | Cookies and tokens live only in `~/.agent-reach/config.yaml` on your machine, mode 600 (owner read/write). They are never uploaded |
| 🛡️ **Safe by default** | `agent-reach install` does not change the system unless you pass `--system` |
| 👀 **Fully open source** | The code is auditable, and so are the upstream tools it installs |
| 🔍 **Dry run** | `agent-reach install --dry-run` previews every action and changes nothing |
| 🧩 **Swappable** | If you do not trust one component, replace that channel file. The others keep working |

### 🍪 Cookie safety

> ⚠️ **Account-ban risk:** Platforms you access with a cookie (Twitter, Xiaohongshu, and others) can detect scripted or API-style calls and restrict or ban the account. Use a **dedicated spare account**, not your main one.

Platforms that need a cookie or a logged-in session (Twitter, Xiaohongshu, Reddit, Facebook, Instagram, and others) should use a spare account:

1. **Ban risk** — the platform may treat non-browser API calls as abuse.
2. **Blast radius** — a cookie is a full login. A spare account limits the damage if it leaks.

### 📦 Install modes

| Mode | Command | When |
|------|------|---------|
| Safe check (default) | `agent-reach install --env=auto` | Every environment. Read-only: it lists what is missing |
| Install system dependencies | `agent-reach install --env=auto --system` | Only after you explicitly allow changes to this machine |
| Compatible safe flag | `agent-reach install --env=auto --safe` | Same as the default |
| Preview only | `agent-reach install --env=auto --dry-run` | See what would happen |
...
This removes `~/.agent-reach/` (including tokens and cookies), the skill files installed into each agent, and the MCP entries in mcporter.

```bash
# Preview only, delete nothing
agent-reach uninstall --dry-run

# Remove skill files, keep token config (for a reinstall)
...
- When people ask for a new channel, I add it
- Each channel should stay **usable, pleasant, and free**
- When a platform changes its anti-bot rules or API, I find another path

Star it so you can find it again next time. ⭐
...
## Business and agent rollouts

I take on custom agent work and production rollouts.

If you have a real process in operations, marketing, research, data, content, or anywhere else that you want an agent to automate, add me on WeChat. You do not need a finished plan. A real workflow and a real problem are enough to figure out whether an agent can help and how.

When you send a friend request, use one of these notes:

- `Business + what you want the agent to do`
- `Builder + what you are building`
- `Group` if you only want the community group
