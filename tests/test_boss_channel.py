...
Boss Zhipin reuses an already logged-in real Chrome through a CDP debug port
(headless is off limits; it triggers code 36 risk control). check() only does
read-only probes: whether boss-agent-cli is installed, whether the CDP port is
up, whether a reusable zhipin tab exists, whether the browser has a login
cookie (wt2), and whether every tab is stuck on an anti-bot security check
page. Each branch returns its own (status, message) and never launches a
browser.

`boss status` only checks the local session.enc and does not mean the CDP
browser is logged in. Layer 4 trusts the browser itself (Storage.getCookies).
"""
...
def test_check_warn_when_ready():
    ...
    # Ready path: check() must mark the backend actually in service (base contract; doctor --json is no longer always null)
...
def test_check_warn_when_cookie_probe_fails():
    ...
    with patch.object(boss_mod, "probe_command", return_value=_ok_probe()), patch.object(
        ...
        status, message = ch.check()
    assert status == "warn"
    assert "login state unknown" in message
    # The path is ready (port up + a tab exists); only the login state is unknown, so the serving backend is still marked
    assert ch.active_backend == ch.backends[0]


def test_check_warn_when_browser_not_logged_in():
    """No wt2 in the browser means logged out, and boss status only represents session.enc."""
    ch = BossChannel()
...
def test_check_warn_when_stuck_on_security_check():
    ...
    assert "does not mean you are logged out" in message
...
# --- WebSocket client for _cdp_zhipin_login_cookie (doctor's read-only wt2 probe) ---

# Fixed urandom produces a fixed Sec-WebSocket-Key so the accept value can be computed ahead of time.
...
class _FakeSock:
    """Fake socket that yields preset byte chunks in order and records sendall."""
...
def _run_ws_probe(monkeypatch, chunks, ws_url="ws://127.0.0.1:9222/devtools/browser/abc"):
    """Run _cdp_zhipin_login_cookie against a fake socket and return (result, fake socket)."""
...
def test_ws_probe_event_frame_before_response(monkeypatch):
    """An event frame (no id) arriving before the response still yields the id==1 response and detects wt2."""
...
def test_ws_probe_accepts_empty_reason_phrase(monkeypatch):
    """An RFC-legal empty reason phrase 'HTTP/1.1 101' (no trailing space) is accepted."""
...
def test_ws_probe_rejects_bogus_1019(monkeypatch):
    """Bogus status 1019 (it contains the substring ' 101 ') is not a successful upgrade."""
...
def test_ws_probe_ipv6_host_header_bracketed(monkeypatch):
    """An IPv6 loopback webSocketDebuggerUrl must send a bracketed Host header."""
