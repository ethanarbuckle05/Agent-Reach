...
class TestCLI:
    ...
    def test_twitter_config_does_not_run_unsafe_verification_or_mutate_env(
        ...
        assert "will not run `twitter status`" in output
    ...
    def test_install_reddit_deps_routes_by_environment(self, monkeypatch):
        """Desktop routes to OpenCLI; a server routes to rdt-cli (pinned git source)."""
        calls = []
    ...
    def test_install_server_dry_run_skips_opencli_only_channels(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "skipping on a server environment: boss, facebook, instagram, opencli" in out
...
class TestVersionCompare:
    ...
    def test_local_ahead_of_release_no_downgrade_prompt(self):
        """During a release window, a local main build that is ahead must not be prompted to downgrade."""
...
class TestWatchVersionCompare:
    def test_watch_does_not_prompt_downgrade(self, monkeypatch, capsys):
        """watch matches check-update: do not prompt an update when the local version is ahead of the release."""
        class R:
            status_code = 200
            headers = {}
