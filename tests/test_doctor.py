...
class TestDoctor:
    def test_check_all_collects_channel_results(self, tmp_config, monkeypatch):
        monkeypatch.setattr(
            ...
            "get_all_channels",
            lambda: [
                _StubChannel("web", "Any web page", 0, "ok", "can fetch web pages", ["requests"],
                             active_backend="requests"),
                _StubChannel("github", "GitHub", 0, "warn", "gh CLI is not installed", ["gh"]),
                _StubChannel("exa_search", "Web semantic search", 1, "off", "mcporter is not configured", ["Exa"]),
            ],
        )
...
def test_stale_active_backend_does_not_leak_into_errored_result(monkeypatch):
    """A previous active_backend on a channel singleton must not leak into this errored result (Codex review)."""
